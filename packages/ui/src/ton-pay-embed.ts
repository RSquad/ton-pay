export interface TonPayEmbedConfig {
  containerId?: string;
...
export interface TonPayEmbedAPI {
  mount: (cfg: Record<string, string | number | boolean>) => void;
  setCallback: (name: string) => void;
...
declare global {
  interface Window {
    TonPayEmbed?: TonPayEmbedAPI;
    payWithTon?: () => void | Promise<void>;
    [key: string]: any;
  }
}
...
(function() {
  ...
  function getBoolParam(name: string, def: boolean): boolean {
    const v = params.get(name);
    if (v == null) return def;
    return v === 'true' || v === '1';
  }

  function getNumParam(name: string, def: number): number {
    const v = params.get(name);
    if (v == null) return def;
    const n = Number(v);
  ...
  const preset = getParam('preset', 'default') as 'default' | 'gradient';
  ...
  const variant = getParam('variant', 'long') as 'long' | 'short';
  ...
  wrap.style.setProperty('--tp-radius', /px$/.test(String(borderRadius)) ? String(borderRadius) : (borderRadius + 'px'));
  wrap.style.setProperty('--tp-font', fontFamily);
  if (width != null) wrap.style.setProperty('--tp-width', cssSize(width) || '');
  if (height != null) wrap.style.setProperty('--tp-height', cssSize(height) || '');
  ...
  function shortenAddress(addr: string | null | undefined, head: number = 4, tail: number = 4): string {
    if (!addr || typeof addr !== "string") return "";
    if (addr.length <= head + tail + 3) return addr;
    return `${addr.slice(0, head)}...${addr.slice(-tail)}`;
  }

  function buildContent(isLoading: boolean): void {
    mainBtn.innerHTML = '';
  ...
  mainBtn.addEventListener('click', function() {
    if (isLoading) return;
    const cb = callbackName && typeof window[callbackName] === 'function' ? (window[callbackName] as () => void | Promise<void>) : null;
    if (!cb) return;
    try {
      const r = cb();
      if (r && typeof r.then === 'function') {
        setLoading(true);
        Promise.resolve(r).finally(() => { setLoading(false); });
      }
    } catch (e) {
      setLoading(false);
    }
  ...
  let arrowBtn: HTMLButtonElement | undefined;
  ...
  function updateMenuContent(): void {
    if (!menu) return;
  ...
  window.addEventListener('tonpay:address', ((e: CustomEvent) => {
  ...
  window.TonPayEmbed = {
    mount: function(cfg: Record<string, string | number | boolean>) {
      try {
    ...
    setCallback: function(name: string) {
      callbackName = name;
    },
    setAddress: function(address: string | null) {
  ...
  if (!window.payWithTon) {
    window.payWithTon = function() {
      if (typeof window[callbackName] === 'function') {
        return (window[callbackName] as () => void | Promise<void>)();
      }
    };
  }
})();
