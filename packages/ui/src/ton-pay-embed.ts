...
(function () {
  ...
  const presetBg =
    preset === 'gradient'
      ? 'linear-gradient(91.69deg, #2A82EB 8.9%, #0355CF 158.29%)'
      : '#0098EA';
  ...
  function setLoading(val: boolean): void {
    ...
    if (isLoading) {
      mainBtn.className =
        mainBtn.className.replace(/\s*loading/g, '') + ' loading';
    } else {
  ...
  mainBtn.addEventListener('click', function () {
    ...
    const cb =
      callbackName && typeof window[callbackName] === 'function'
        ? (window[callbackName] as () => void | Promise<void>)
        : null;
    if (!cb) return;
    try {
      const r = cb();
  ...
  function syncMenuVisibility(): void {
    ...
    mainBtn.className =
      mainBtn.className.replace(/\s*(with-menu|no-menu)/g, '') +
      ' ' +
      (showMenu && currentAddress ? 'with-menu' : 'no-menu');
